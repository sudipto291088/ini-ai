...
# =========================
APP_TITLE = "Ini.ai"
API_BASE = os.getenv("INI_API_BASE", "http://127.0.0.1:8000")
DEFAULT_DOMAIN = "Artificial Intelligence"

# Modes (these drive backend payloads)
MODE_DEEP = "deep"
MODE_HIGH = "high"
MODE_QUIZ = "quiz"
ALL_MODES = [MODE_DEEP, MODE_HIGH, MODE_QUIZ]
...
def normalize_mojibake(s: str) -> str:
    """
    Fix common mojibake from Windows consoles / copy-paste.
    Safe: if it fails, returns original.
    """
    if not s:
        return ""
    try:
        # Attempt to re-decode if it looks like latin-1 mojibake
...
def normalize_whitespace_for_readability(s: str) -> str:
    """
    Light cleanup that improves spacing/indent consistency without changing meaning.
    """
    if not s:
        return ""
    s = s.replace("\r\n", "\n").replace("\r", "\n")
    ...
    # Avoid extreme vertical gaps
    s = re.sub(r"\n{4,}", "\n\n\n", s)
    return s.strip()


def strip_ui_artifacts(s: str) -> str:
    """Remove any UI/debug strings that accidentally got persisted inside chat text."""
    if not s:
        return ""
    # Legacy: HTML blocks that were once appended into messages
    s = re.sub(r"<div\s+class=\"mode_hint_left\">[\s\S]*?</div>", "", s, flags=re.IGNORECASE)
    s = re.sub(r"<div\s+class=\"mode_hint_right\">[\s\S]*?</div>", "", s, flags=re.IGNORECASE)

    # Legacy: debug widget keys showing in chat
    s = s.replace("SEND_HIDDEN", "").replace("ini_hidden_send", "").replace("uib_text", "")

    # Legacy: 3-line mode list leaked into user bubble
    s = re.sub(r"(?m)^\s*[►▶].*Deep.*\n\s*[◎◉].*Overview.*\n\s*[?].*Quiz.*\s*$", "", s)
...
def _init_state() -> None:
    ...
    # Sessions store for "My New Learning"
    if "learning_sessions" not in ss:
        ss.learning_sessions = []

    if "active_learning_session_id" not in ss:
        ss.active_learning_session_id = None

    # Input + mode selection
    if "uib_mode" not in ss:
        ss.uib_mode = MODE_DEEP

    if "uib_text" not in ss:
        ss.uib_text = ""

    if "last_sent" not in ss:
        ss.last_sent = 0.0


def _new_learning_session() -> None:
    sess_id = f"SESSION.{datetime.now().strftime('%b %d.%Y')}"
    sess = {
        "id": sess_id,
        "created_at": time.time(),
        "messages": [],
    }
    st.session_state.learning_sessions.append(sess)
    st.session_state.active_learning_session_id = sess_id


def _get_active_learning_session() -> Dict[str, Any]:
    ss = st.session_state
    sid = ss.active_learning_session_id
    if sid is None:
        _new_learning_session()
        sid = ss.active_learning_session_id

    for s in ss.learning_sessions:
        if s["id"] == sid:
            return s
    ...
    for s in ss.learning_sessions:
        if s["id"] == sid:
            return s
    # fallback
    return ss.learning_sessions[-1]


# =========================
# Backend calls
# =========================
def api_study_ai(topic: str, mode: str, domain: str = DEFAULT_DOMAIN) -> Dict[str, Any]:
    payload = {"topic": topic, "mode": mode, "domain": domain}
    url = f"{API_BASE}/study/ai"
...
<style>
/* Layout */
.ini_wrap { max-width: 1100px; margin: 0 auto; }

/* Chat bubble */
.chat_row { display:flex; width: 100%; margin: 10px 0; }
.chat_row.user { justify-content:flex-end; }
.chat_row.assistant { justify-content:flex-start; }
...
.chat_bubble.user { background: rgba(240,248,255,0.85); }
.chat_bubble.assistant { background: rgba(255,255,255,0.95); }
...
.badge.user { background: #ff5a5a; }
.badge.assistant { background: #f0a500; }

/* Mode hint line (outside bubble) */
.mode_hint_line {
  font-size: 12px;
  opacity: 0.70;
  margin-top: -6px;
...
.mode_hint_line.user { text-align: right; margin-right: 6px; }
.mode_hint_line.assistant { text-align: left; }

/* Input bar area */
.input_row { display:flex; gap: 10px; align-items: center; }
.input_row .stTextInput { flex: 1; }
</style>
        """,
        unsafe_allow_html=True,
    )
...
def render_learning_messages(sess: Dict[str, Any]) -> None:
    for msg in sess["messages"]:
        ...
        text = normalize_whitespace_for_readability(text)
        text = strip_ui_artifacts(text)

        # IMPORTANT: escape to prevent any stored HTML/UI fragments from breaking the bubbles
        ...
        if ts:
            try:
                ts_str = datetime.fromtimestamp(ts).strftime("%a, %b %d • %I:%M %p")
            except Exception:
                ts_str = ""
        ...
        if role == "user":
            msg_html = f"""
<div class="chat_row user">
  <div class="chat_bubble user">
    <div class="chat_meta">
      <div><span class="badge user">fac</span><b>You</b></div>
      <div>{ts_str}</div>
    </div>
    <div class="chat_text">{safe_text}</div>
  </div>
</div>
"""
            st.markdown(msg_html, unsafe_allow_html=True)

            # Optional: show selected mode UNDER the bubble (not inside)
            if mode:
                st.markdown(
                    f'<div class="mode_hint_line user">• {html.escape(mode)} </div>',
                    unsafe_allow_html=True,
                )

        else:
            msg_html = f"""
<div class="chat_row assistant">
  <div class="chat_bubble assistant">
    <div class="chat_meta">
      <div><span class="badge assistant">sm</span><b>Ini</b></div>
      <div>{ts_str}</div>
    </div>
    <div class="chat_text">{safe_text}</div>
  </div>
...
# Actions
# =========================
def _send_learning(topic: str, mode: str) -> None:
    sess = _get_active_learning_session()
    ...
    # Call backend
    try:
        r = api_study_ai(topic=topic, mode=mode, domain=DEFAULT_DOMAIN)
        ans = r.get("answer", "")
        ...
        footer = ""
        if incomplete:
            footer = f"\n\n(incomplete: True | stop_reason: {stop_reason})"
...
def page_my_new_learning() -> None:
    ...
    # Input row
    st.markdown("<div class='ini_wrap'>", unsafe_allow_html=True)

    col_in, col_eye, col_q, col_send = st.columns([12, 1, 1, 1])

    with col_in:
        topic = st.text_input(" ", key="uib_text", placeholder="Type your topic/question...")

    with col_eye:
        if st.button("◎", help="Overview"):
            st.session_state.uib_mode = MODE_HIGH

    with col_q:
        if st.button("?", help="Quiz"):
            st.session_state.uib_mode = MODE_QUIZ

    with col_send:
        if st.button("▶", help="Send"):
            st.session_state.uib_mode = st.session_state.uib_mode or MODE_DEEP
            if topic.strip():
                _send_learning(topic.strip(), st.session_state.uib_mode)
                st.session_state.uib_text = ""

    # Hint text
    mode_label = {
        MODE_DEEP: "Deep (default)",
        MODE_HIGH: "Overview",
        MODE_QUIZ: "Quiz",
    }.get(st.session_state.uib_mode, "Deep (default)")
...
def sidebar_nav() -> str:
    ...
    st.sidebar.caption("v0 • AI Tutor")

    st.sidebar.markdown("#### Navigation")
    nav = st.sidebar.radio(
        " ",
        options=["New Chat", "My New Learning", "New Project"],
        index=1,
        label_visibility="collapsed",
    )
    ...
    # sessions list
    ss = st.session_state
    if ss.learning_sessions:
        for s in ss.learning_sessions[-6:]:
            sid = s["id"]
            st.sidebar.write(f"• {sid}")
